...
function advanceToNextDay()
    day = day + 1
    actionPoints = 10 -- 重置行动点
    water = water + 5 -- 每天增加一些水资源
    print("Advancing to next day. Day: " .. day .. ", Action Points reset to 10.")
end
...
function love.keypressed(key)
    if gameState == "menu" then
        if key == "return" then
            gameState = "game"
            gamebackground = land4
            print("Changed background to land4")
        elseif key == "h" or key == "H" then
            previousGameState = gameState
            gameState = "help"
            print("Help screen opened from menu")
        end
    elseif gameState == "game" then
        if key == "q" then
            selectedSeed = "wheat"
        ...
        elseif key == "h" or key == "H" then
            previousGameState = gameState
            gameState = "help"
            print("Help screen opened from game")
        end
    elseif gameState == "shop" or gameState == "warehouse" or gameState == "help" then
        if key == "escape" then
            if gameState == "help" and previousGameState then
